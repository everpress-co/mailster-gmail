 ...
 */
namespace Mailster\phpseclib\Net\SFTP;

use Mailster\phpseclib\Crypt\RSA;
use Mailster\phpseclib\Net\SFTP;
/**
 * SFTP Stream Wrapper
 *
 * @package SFTP
...
class Stream
{
    ...
    function _parse_path($path)
    {
        ...
        if ($host[0] == '$') {
            $host = \substr($host, 1);
            global ${$host};
            if (${$host} instanceof \Mailster\phpseclib\Net\SFTP === \false) {
                return \false;
            }
            $this->sftp = ${$host};
        } else {
            ...
            if (isset($sftp) && $sftp instanceof \Mailster\phpseclib\Net\SFTP) {
                $this->sftp = $sftp;
                return $path;
            }
            ...
            if (isset($context[$scheme]['privkey']) && $context[$scheme]['privkey'] instanceof \Mailster\phpseclib\Crypt\RSA) {
            ...
            if (isset(self::$instances[$host][$port][$user][(string) $pass])) {
                ...
                $this->sftp = new \Mailster\phpseclib\Net\SFTP($host, $port);
    ...
    function _stream_write($data)
    {
        ...
        $result = $this->sftp->put($this->path, $data, \Mailster\phpseclib\Net\SFTP::SOURCE_STRING, $this->pos);
        if (isset($this->notification) && \is_callable($this->notification)) {
            if (!$result) {
                \call_user_func($this->notification, \STREAM_NOTIFY_FAILURE, \STREAM_NOTIFY_SEVERITY_ERR, $this->sftp->getLastSFTPError(), NET_SFTP_OPEN, 0, 0);
                return 0;
