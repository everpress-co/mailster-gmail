...
use Mailster\phpseclib\Crypt\DES;
...
class SSH1
{
    ...
    function _connect()
    {
        ...
        $server_key_public_exponent = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($response[self::RESPONSE_DATA], \ceil($temp['len'] / 8)), 256);
        $this->server_key_public_exponent = $server_key_public_exponent;
        if (\strlen($response[self::RESPONSE_DATA]) < 2) {
            return \false;
        }
        $temp = \unpack('nlen', $this->_string_shift($response[self::RESPONSE_DATA], 2));
        $server_key_public_modulus = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($response[self::RESPONSE_DATA], \ceil($temp['len'] / 8)), 256);
        ...
        $temp = \unpack('nlen', $this->_string_shift($response[self::RESPONSE_DATA], 2));
        $host_key_public_exponent = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($response[self::RESPONSE_DATA], \ceil($temp['len'] / 8)), 256);
        $this->host_key_public_exponent = $host_key_public_exponent;
        if (\strlen($response[self::RESPONSE_DATA]) < 2) {
            return \false;
        }
        $temp = \unpack('nlen', $this->_string_shift($response[self::RESPONSE_DATA], 2));
        $host_key_public_modulus = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($response[self::RESPONSE_DATA], \ceil($temp['len'] / 8)), 256);
        ...
        $session_key = \Mailster\phpseclib\Crypt\Random::string(32);
        ...
        switch ($cipher) {
            ...
            case self::CIPHER_DES:
                $this->crypto = new \Mailster\phpseclib\Crypt\DES();
                $this->crypto->disablePadding();
                $this->crypto->enableContinuousBuffer();
                $this->crypto->setKey(\substr($session_key, 0, 8));
                break;
            case self::CIPHER_3DES:
                $this->crypto = new \Mailster\phpseclib\Crypt\TripleDES(\Mailster\phpseclib\Crypt\TripleDES::MODE_3CBC);
                $this->crypto->disablePadding();
                $this->crypto->enableContinuousBuffer();
    ...
    function _send_binary_packet($data)
    {
        ...
        $padding = \Mailster\phpseclib\Crypt\Random::string(8 - ($length & 7));
        $orig = $data;
        $data = $padding . $data;
    ...
    function _rsa_crypt($m, $key)
    {
        ...
        */
        ...
        while (\strlen($random) != $length) {
            $block = \Mailster\phpseclib\Crypt\Random::string($length - \strlen($random));
            $block = \str_replace("\0", '', $block);
            $random .= $block;
        }
        $temp = \chr(0) . \chr(2) . $random . \chr(0) . $m;
        $m = new \Mailster\phpseclib\Math\BigInteger($temp, 256);
