...
class RSA
{
    ...
    function __construct()
    {
        ...
        $this->zero = new \Mailster\phpseclib\Math\BigInteger();
        $this->one = new \Mailster\phpseclib\Math\BigInteger(1);
        $this->hash = new \Mailster\phpseclib\Crypt\Hash('sha1');
        $this->hLen = $this->hash->getLength();
        $this->hashName = 'sha1';
        $this->mgfHash = new \Mailster\phpseclib\Crypt\Hash('sha1');
    ...
    function createKey($bits = 1024, $timeout = \false, $partial = array())
    {
        ...
        static $e;
        if (!isset($e)) {
            $e = new \Mailster\phpseclib\Math\BigInteger(CRYPT_RSA_EXPONENT);
        }
        ...
        $generator = new \Mailster\phpseclib\Math\BigInteger();
        $n = $this->one->copy();
        if (!empty($partial)) {
        ...
        switch ($this->privateKeyFormat) {
            ...
            case self::PRIVATE_FORMAT_PUTTY:
                ...
                if (empty($this->password) && !\is_string($this->password)) {
                    ...
                    $private .= \Mailster\phpseclib\Crypt\Random::string(16 - (\strlen($private) & 15));
                    $source .= \pack('Na*', \strlen($private), $private);
                    $sequence = 0;
                    $symkey = '';
                    ...
                    $crypto = new \Mailster\phpseclib\Crypt\AES();
            ...
            case self::PRIVATE_FORMAT_OPENSSH:
                ...
                $checkint = \Mailster\phpseclib\Crypt\Random::string(4);
            ...
            default:
                ...
                if ($this->privateKeyFormat == self::PRIVATE_FORMAT_PKCS8) {
                    ...
                    $RSAPrivateKey = \pack('Ca*a*', self::ASN1_SEQUENCE, $this->_encodeLength(\strlen($RSAPrivateKey)), $RSAPrivateKey);
                    if (!empty($this->password) || \is_string($this->password)) {
                        $salt = \Mailster\phpseclib\Crypt\Random::string(8);
                        $iterationCount = 2048;
                        $crypto = new \Mailster\phpseclib\Crypt\DES();
                ...
                if (!empty($this->password) || \is_string($this->password)) {
                    $iv = \Mailster\phpseclib\Crypt\Random::string(8);
                    $symkey = \pack('H*', \md5($this->password . $iv));
                    // symkey is short for symmetric key
                    $symkey .= \substr(\pack('H*', \md5($symkey . $this->password . $iv)), 0, 8);
                    $des = new \Mailster\phpseclib\Crypt\TripleDES();
                    $des->setKey($symkey);
                    $des->setIV($iv);
        ...
        switch ($type) {
            ...
            case self::PUBLIC_FORMAT_PKCS1:
                ...
                if (\preg_match('#DEK-Info: (.+),(.+)#', $key, $matches)) {
                    ...
                    switch ($matches[1]) {
                        ...
                        case 'AES-128-CBC':
                            ...
                            break;
                        case 'DES-EDE3-CFB':
                            $crypto = new \Mailster\phpseclib\Crypt\TripleDES(\Mailster\phpseclib\Crypt\Base::MODE_CFB);
                            break;
                        case 'DES-EDE3-CBC':
                            $symkey = \substr($symkey, 0, 24);
                            $crypto = new \Mailster\phpseclib\Crypt\TripleDES();
                            break;
                        case 'DES-CBC':
                            $crypto = new \Mailster\phpseclib\Crypt\DES();
                            break;
                ...
                if (\strlen($temp) != 1 || \ord($temp) > 2) {
                    $components['modulus'] = new \Mailster\phpseclib\Math\BigInteger($temp, 256);
                    $this->_string_shift($key);
                    // skip over self::ASN1_INTEGER
                    $length = $this->_decodeLength($key);
                    $components[$type == self::PUBLIC_FORMAT_PKCS1 ? 'publicExponent' : 'privateExponent'] = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256);
                    return $components;
                }
                ...
                $length = $this->_decodeLength($key);
                $components['modulus'] = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256);
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['publicExponent'] = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256);
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['privateExponent'] = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256);
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['primes'] = array(1 => new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256));
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['primes'][] = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256);
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['exponents'] = array(1 => new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256));
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['exponents'][] = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256);
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['coefficients'] = array(2 => new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256));
                if (!empty($key)) {
                    if (\ord($this->_string_shift($key)) != self::ASN1_SEQUENCE) {
                        return \false;
                    }
                    ...
                    while (!empty($key)) {
                        ...
                        $this->_string_shift($key);
                        $length = $this->_decodeLength($key);
                        $components['coefficients'][] = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256);
                    }
                }
                return $components;
            case self::PUBLIC_FORMAT_OPENSSH:
                ...
                $publicExponent = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($key, $length), -256);
                if (\strlen($key) <= 4) {
                    return \false;
                }
                \extract(\unpack('Nlength', $this->_string_shift($key, 4)));
                $modulus = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($key, $length), -256);
                if ($cleanup && \strlen($key)) {
                    if (\strlen($key) <= 4) {
                        return \false;
                    }
                    \extract(\unpack('Nlength', $this->_string_shift($key, 4)));
                    $realModulus = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($key, $length), -256);
            ...
            case self::PRIVATE_FORMAT_PUTTY:
                ...
                $components['publicExponent'] = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($public, $length), -256);
                \extract(\unpack('Nlength', $this->_string_shift($public, 4)));
                $components['modulus'] = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($public, $length), -256);
                ...
                $components['privateExponent'] = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($private, $length), -256);
                \extract(\unpack('Nlength', $this->_string_shift($private, 4)));
                if (\strlen($private) < $length) {
                    return \false;
                }
                $components['primes'] = array(1 => new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($private, $length), -256));
                \extract(\unpack('Nlength', $this->_string_shift($private, 4)));
                if (\strlen($private) < $length) {
                    return \false;
                }
                $components['primes'][] = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($private, $length), -256);
                ...
                $components['coefficients'] = array(2 => new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($private, $length), -256));
                return $components;
            case self::PRIVATE_FORMAT_OPENSSH:
                $components = array();
                $decoded = $this->_extractBER($key);
                ...
                foreach ($values as &$value) {
                    ...
                    $value = new \Mailster\phpseclib\Math\BigInteger($this->_string_shift($paddedKey, $length), -256);
                }
                \extract(\unpack('Nlength', $this->_string_shift($paddedKey, 4)));
                if (\strlen($paddedKey) < $length) {
                    return \false;
    ...
    function _stop_element_handler($parser, $name)
    {
        if (isset($this->current)) {
            $this->current = new \Mailster\phpseclib\Math\BigInteger(\base64_decode($this->current), 256);
            unset($this->current);
        }
    }
    /**
    ...
    function loadKey($key, $type = \false)
    {
        if ($key instanceof \Mailster\phpseclib\Crypt\RSA) {
            ...
            if (\is_object($key->hash)) {
                $this->hash = new \Mailster\phpseclib\Crypt\Hash($key->hash->getHash());
            }
            if (\is_object($key->mgfHash)) {
                $this->mgfHash = new \Mailster\phpseclib\Crypt\Hash($key->mgfHash->getHash());
            }
    ...
    function getPublicKeyFingerprint($algorithm = 'md5')
    {
        ...
        switch ($algorithm) {
            case 'sha256':
                $hash = new \Mailster\phpseclib\Crypt\Hash('sha256');
    ...
    function _generateMinMax($bits)
    {
        ...
        return array('min' => new \Mailster\phpseclib\Math\BigInteger($min, 256), 'max' => new \Mailster\phpseclib\Math\BigInteger($max, 256));
    }
    /**
    ...
    function setHash($hash)
    {
        ...
        switch ($hash) {
            ...
            case 'sha384':
            case 'sha512':
                $this->hash = new \Mailster\phpseclib\Crypt\Hash($hash);
                $this->hashName = $hash;
                break;
            default:
                $this->hash = new \Mailster\phpseclib\Crypt\Hash('sha1');
    ...
    function setMGFHash($hash)
    {
        ...
        switch ($hash) {
            ...
            case 'sha384':
            case 'sha512':
                $this->mgfHash = new \Mailster\phpseclib\Crypt\Hash($hash);
                break;
            default:
                $this->mgfHash = new \Mailster\phpseclib\Crypt\Hash('sha1');
        }
    ...
    function _os2ip($x)
    {
        return new \Mailster\phpseclib\Math\BigInteger($x, 256);
    }
    /**
    ...
    function _rsaes_oaep_encrypt($m, $l = '')
    {
        ...
        $db = $lHash . $ps . \chr(1) . $m;
        $seed = \Mailster\phpseclib\Crypt\Random::string($this->hLen);
    ...
    function _rsaes_pkcs1_v1_5_encrypt($m)
    {
        ...
        while (\strlen($ps) != $psLen) {
            $temp = \Mailster\phpseclib\Crypt\Random::string($psLen - \strlen($ps));
    ...
    function _emsa_pss_encode($m, $emBits)
    {
        ...
        $salt = \Mailster\phpseclib\Crypt\Random::string($sLen);
