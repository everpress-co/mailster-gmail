...
use DateTimeZone;
use Mailster\Monolog\Handler\HandlerInterface;
 ...
 */
class Logger implements \Mailster\Psr\Log\LoggerInterface, \Mailster\Monolog\ResettableInterface
{
    /**
     ...
     */
    public function pushHandler(\Mailster\Monolog\Handler\HandlerInterface $handler) : self
    {
     ...
     */
    public function popHandler() : \Mailster\Monolog\Handler\HandlerInterface
    {
        if (!$this->handlers) {
    ...
    public function addRecord(int $level, string $message, array $context = []) : bool
    {
        ...
        $record = ['message' => $message, 'context' => $context, 'level' => $level, 'level_name' => $levelName, 'channel' => $this->name, 'datetime' => new \Mailster\Monolog\DateTimeImmutable($this->microsecondTimestamps, $this->timezone), 'extra' => []];
        try {
    ...
    public function reset() : void
    {
        ...
        foreach ($this->processors as $processor) {
            if ($processor instanceof \Mailster\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
    ...
    public static function getLevelName(int $level) : string
    {
        if (!isset(static::$levels[$level])) {
            throw new \Mailster\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
    ...
    public static function toMonologLevel($level) : int
    {
        ...
        if (!\is_int($level)) {
            throw new \Mailster\Psr\Log\InvalidArgumentException('Level "' . \var_export($level, \true) . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
        return $level;
    }
    /**
