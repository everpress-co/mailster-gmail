...
use Mailster\Monolog\Formatter\JsonFormatter;
use Mailster\PhpAmqpLib\Message\AMQPMessage;
use Mailster\PhpAmqpLib\Channel\AMQPChannel;
use AMQPExchange;
class AmqpHandler extends \Mailster\Monolog\Handler\AbstractProcessingHandler
{
    /**
     ...
     */
    public function __construct($exchange, ?string $exchangeName = null, $level = \Mailster\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if ($exchange instanceof \Mailster\PhpAmqpLib\Channel\AMQPChannel) {
    ...
    private function createAmqpMessage(string $data) : \Mailster\PhpAmqpLib\Message\AMQPMessage
    {
        return new \Mailster\PhpAmqpLib\Message\AMQPMessage($data, ['delivery_mode' => 2, 'content_type' => 'application/json']);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return new \Mailster\Monolog\Formatter\JsonFormatter(\Mailster\Monolog\Formatter\JsonFormatter::BATCH_MODE_JSON, \false);
    }
}
