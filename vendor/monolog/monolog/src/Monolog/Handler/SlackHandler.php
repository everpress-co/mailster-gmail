...
use Mailster\Monolog\Handler\Slack\SlackRecord;
/**
 ...
 */
class SlackHandler extends \Mailster\Monolog\Handler\SocketHandler
{
    /**
     * Slack API token
     * @var string
     ...
     */
    public function __construct(string $token, string $channel, ?string $username = null, bool $useAttachment = \true, ?string $iconEmoji = null, $level = \Mailster\Monolog\Logger::CRITICAL, bool $bubble = \true, bool $useShortAttachment = \false, bool $includeContextAndExtra = \false, array $excludeFields = array())
    {
        if (!\extension_loaded('openssl')) {
            throw new \Mailster\Monolog\Handler\MissingExtensionException('The OpenSSL PHP extension is required to use the SlackHandler');
        }
        parent::__construct('ssl://slack.com:443', $level, $bubble);
        $this->slackRecord = new \Mailster\Monolog\Handler\Slack\SlackRecord($channel, $username, $useAttachment, $iconEmoji, $useShortAttachment, $includeContextAndExtra, $excludeFields);
        $this->token = $token;
    }
    public function getSlackRecord() : \Mailster\Monolog\Handler\Slack\SlackRecord
    {
    ...
    protected function prepareContentData(array $record) : array
    {
        ...
        if (!empty($dataArray['attachments'])) {
            $dataArray['attachments'] = \Mailster\Monolog\Utils::jsonEncode($dataArray['attachments']);
        }
        return $dataArray;
    }
    /**
