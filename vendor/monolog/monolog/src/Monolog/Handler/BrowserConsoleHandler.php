 ...
 */
class BrowserConsoleHandler extends \Mailster\Monolog\Handler\AbstractProcessingHandler
{
    ...
    {
        return new \Mailster\Monolog\Formatter\LineFormatter('[[%channel%]]{macro: autolabel} [[%level_name%]]{font-weight: bold} %message%');
    }
    /**
     * {@inheritDoc}
     */
    ...
    private static function handleStyles(string $formatted) : array
    {
        ...
        foreach (\array_reverse($matches) as $match) {
            ...
            $pos = $match[0][1];
            $format = \Mailster\Monolog\Utils::substr($format, 0, $pos) . '%c' . $match[1][0] . '%c' . \Mailster\Monolog\Utils::substr($format, $pos + \strlen($match[0][0]));
        }
