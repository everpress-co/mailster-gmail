...
use Mailster\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy;
use Mailster\Monolog\Handler\FingersCrossed\ActivationStrategyInterface;
 ...
 */
class FingersCrossedHandler extends \Mailster\Monolog\Handler\Handler implements \Mailster\Monolog\Handler\ProcessableHandlerInterface, \Mailster\Monolog\ResettableInterface, \Mailster\Monolog\Handler\FormattableHandlerInterface
{
    ...
    public function __construct($handler, $activationStrategy = null, int $bufferSize = 0, bool $bubble = \true, bool $stopBuffering = \true, $passthruLevel = null)
    {
        if (null === $activationStrategy) {
            $activationStrategy = new \Mailster\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy(\Mailster\Monolog\Logger::WARNING);
        }
        // convert simple int activationStrategy to an object
        if (!$activationStrategy instanceof \Mailster\Monolog\Handler\FingersCrossed\ActivationStrategyInterface) {
            $activationStrategy = new \Mailster\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy($activationStrategy);
        }
        ...
        if ($passthruLevel !== null) {
            $this->passthruLevel = \Mailster\Monolog\Logger::toMonologLevel($passthruLevel);
        }
    ...
    public function reset()
    {
        ...
        if ($this->getHandler() instanceof \Mailster\Monolog\ResettableInterface) {
