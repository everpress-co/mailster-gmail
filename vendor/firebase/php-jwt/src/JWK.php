...
class JWK
{
    ...
    public static function parseKeySet(array $jwks)
    {
        ...
        if (empty($jwks['keys'])) {
            throw new \Mailster\Firebase\JWT\InvalidArgumentException('JWK Set did not contain any keys');
        }
    ...
    {
        if (empty($jwk)) {
            throw new \Mailster\Firebase\JWT\InvalidArgumentException('JWK must not be empty');
        }
        if (!isset($jwk['kty'])) {
    ...
    {
        $modulus = \Mailster\Firebase\JWT\JWT::urlsafeB64Decode($n);
        $publicExponent = \Mailster\Firebase\JWT\JWT::urlsafeB64Decode($e);
