 ...
 */
use Mailster\Google\Auth\ApplicationDefaultCredentials;
use Mailster\Google\Auth\Cache\MemoryCacheItemPool;
...
use Mailster\Google\Auth\Credentials\UserRefreshCredentials;
use Mailster\GuzzleHttp\Client;
use Mailster\GuzzleHttp\ClientInterface;
use Mailster\GuzzleHttp\Ring\Client\StreamHandler;
...
use Mailster\Monolog\Handler\StreamHandler as MonologStreamHandler;
use Mailster\Monolog\Handler\SyslogHandler as MonologSyslogHandler;
/**
...
class Google_Client
{
    ...
    public function fetchAccessTokenWithAuthCode($code)
    {
        ...
        $httpHandler = \Mailster\Google\Auth\HttpHandler\HttpHandlerFactory::build($this->getHttpClient());
     ...
     * @return array access token
     */
    public function fetchAccessTokenWithAssertion(\Mailster\GuzzleHttp\ClientInterface $authHttp = null)
    {
        ...
        $httpHandler = \Mailster\Google\Auth\HttpHandler\HttpHandlerFactory::build($authHttp);
     ...
     */
    public function authorize(\Mailster\GuzzleHttp\ClientInterface $http = null)
    {
        $credentials = null;
        $token = null;
        $scopes = null;
    ...
    {
        $tokenRevoker = new \Mailster\Google_AccessToken_Revoke($this->getHttpClient());
    ...
    {
        $tokenVerifier = new \Mailster\Google_AccessToken_Verify($this->getHttpClient(), $this->getCache(), $this->config['jwt']);
        if (null === $idToken) {
     ...
     */
    public function execute(\Mailster\Psr\Http\Message\RequestInterface $request, $expectedClass = null)
    {
        ...
        $http = $this->authorize();
        return \Mailster\Google_Http_REST::execute($http, $request, $expectedClass, $this->config['retry'], $this->config['retry_map']);
    }
    /**
    ...
    {
        $auth = new \Mailster\Google\Auth\OAuth2(['clientId' => $this->getClientId(), 'clientSecret' => $this->getClientSecret(), 'authorizationUri' => self::OAUTH2_AUTH_URL, 'tokenCredentialUri' => self::OAUTH2_TOKEN_URI, 'redirectUri' => $this->getRedirectUri(), 'issuer' => $this->config['client_id'], 'signingKey' => $this->config['signing_key'], 'signingAlgorithm' => $this->config['signing_algorithm']]);
        return $auth;
    }
    /**
     ...
     */
    public function setCache(\Mailster\Psr\Cache\CacheItemPoolInterface $cache)
    {
        $this->cache = $cache;
    }
    /**
    ...
    {
        $logger = new \Mailster\Monolog\Logger('google-api-php-client');
        if ($this->isAppEngine()) {
            $handler = new \Mailster\Monolog\Handler\SyslogHandler('app', \LOG_USER, \Mailster\Monolog\Logger::NOTICE);
        } else {
            $handler = new \Mailster\Monolog\Handler\StreamHandler('php://stderr', \Mailster\Monolog\Logger::NOTICE);
        }
    ...
    {
        return new \Mailster\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
     * Set the Http Client object
     * @param GuzzleHttp\ClientInterface $http
     */
    public function setHttpClient(\Mailster\GuzzleHttp\ClientInterface $http)
    {
        $this->http = $http;
    }
    /**
    ...
    protected function createDefaultHttpClient()
    {
        ...
        if ('5' === $version[0]) {
            ...
            if ($this->isAppEngine()) {
                ...
                $options['handler'] = new \Mailster\GuzzleHttp\Ring\Client\StreamHandler();
        ...
        return new \Mailster\GuzzleHttp\Client($options);
    }
    private function createApplicationDefaultCredentials()
    {
        ...
        if ($signingKey) {
            ...
            $credentials = \Mailster\Google\Auth\CredentialsLoader::makeCredentials($scopes, $serviceAccountCredentials);
        } else {
            $credentials = \Mailster\Google\Auth\ApplicationDefaultCredentials::getCredentials($scopes, null, null, null, $this->config['quota_project']);
        }
        // for service account domain-wide authority (impersonating a user)
        // @see https://developers.google.com/identity/protocols/OAuth2ServiceAccount
        if ($sub) {
            if (!$credentials instanceof \Mailster\Google\Auth\Credentials\ServiceAccountCredentials) {
    ...
    protected function getAuthHandler()
    {
        ...
        return \Mailster\Google_AuthHandler_AuthHandlerFactory::build($this->getCache(), $this->config['cache_config']);
    }
    private function createUserRefreshCredentials($scope, $refreshToken)
    {
        ...
        return new \Mailster\Google\Auth\Credentials\UserRefreshCredentials($scope, $creds);
    }
}
/**
 * The Google API Client
 * https://github.com/google/google-api-php-client
 */
\class_alias('Mailster\\Google_Client', 'Google_Client', \false);
