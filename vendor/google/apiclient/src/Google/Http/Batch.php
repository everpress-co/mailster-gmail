...
class Google_Http_Batch
{
    ...
    private $rootUrl;
    private $batchPath;
    public function __construct(\Mailster\Google_Client $client, $boundary = \false, $rootUrl = null, $batchPath = null)
    {
        $this->client = $client;
    ...
    public function add(\Mailster\Psr\Http\Message\RequestInterface $request, $key = \false)
    {
        if (\false == $key) {
            $key = \mt_rand();
        }
...
EOF;
        ...
        $request = new \Mailster\GuzzleHttp\Psr7\Request('POST', $url, $headers, $body);
        $response = $this->client->execute($request);
        return $this->parseResponse($response, $classes);
    }
    public function parseResponse(\Mailster\Psr\Http\Message\ResponseInterface $response, $classes = array())
    {
        ...
        if (!empty($body)) {
            ...
            foreach ($parts as $i => $part) {
                ...
                if (!empty($part)) {
                    ...
                    $response = new \Mailster\GuzzleHttp\Psr7\Response($status, $partHeaders, \Mailster\GuzzleHttp\Psr7\stream_for($partBody));
                    // Need content id.
                    $key = $headers['content-id'];
                    try {
                        $response = \Mailster\Google_Http_REST::decodeHttpResponse($response, $requests[$i - 1]);
 ...
 * class.
 */
\class_alias('Mailster\\Google_Http_Batch', 'Google_Http_Batch', \false);
