 ...
 */
class UserRefreshCredentials extends \Mailster\Google\Auth\CredentialsLoader implements \Mailster\Google\Auth\GetQuotaProjectInterface
{
    ...
    public function __construct($scope, $jsonKey)
    {
        ...
        $this->auth = new \Mailster\Google\Auth\OAuth2(['clientId' => $jsonKey['client_id'], 'clientSecret' => $jsonKey['client_secret'], 'refresh_token' => $jsonKey['refresh_token'], 'scope' => $scope, 'tokenCredentialUri' => self::TOKEN_CREDENTIAL_URI]);
        if (\array_key_exists('quota_project', $jsonKey)) {
            $this->quotaProject = (string) $jsonKey['quota_project'];
        }
        if ($jsonKey['client_id'] === self::CLOUD_SDK_CLIENT_ID && \is_null($this->quotaProject) && \getenv(self::SUPPRESS_CLOUD_SDK_CREDS_WARNING_ENV) !== 'true') {
