...
use DateTime;
use Exception;
use Mailster\Firebase\JWT\ExpiredException;
use Mailster\Firebase\JWT\JWT;
use Mailster\Firebase\JWT\SignatureInvalidException;
...
use Mailster\SimpleJWT\InvalidTokenException;
use Mailster\SimpleJWT\JWT as SimpleJWT;
use Mailster\SimpleJWT\Keys\KeyFactory;
use Mailster\SimpleJWT\Keys\KeySet;
...
class AccessToken
{
     ...
     */
    public function __construct(callable $httpHandler = null, \Mailster\Psr\Cache\CacheItemPoolInterface $cache = null)
    {
        ...
        $this->cache = $cache ?: new \Mailster\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
    ...
    public function verify($token, array $options = [])
    {
        ...
        } catch (\Mailster\Firebase\JWT\ExpiredException $e) {
            // firebase/php-jwt 3+
        } catch (\Mailster\ExpiredException $e) {
            // firebase/php-jwt 2
        } catch (\Mailster\Firebase\JWT\SignatureInvalidException $e) {
            // firebase/php-jwt 3+
        } catch (\Mailster\SignatureInvalidException $e) {
            // firebase/php-jwt 2
        } catch (\Mailster\SimpleJWT\InvalidTokenException $e) {
            // simplejwt
        } catch (\Mailster\Google\Auth\DomainException $e) {
    ...
    {
        $this->checkSimpleJwt();
        $jwkset = new \Mailster\SimpleJWT\Keys\KeySet();
        foreach ($certs as $cert) {
            $jwkset->add(\Mailster\SimpleJWT\Keys\KeyFactory::create($cert, 'php'));
        }
    ...
    private function verifyRs256($token, array $certs, $audience = null, $issuer = null)
    {
        ...
        foreach ($certs as $cert) {
            ...
            $rsa->loadKey(['n' => new \Mailster\phpseclib\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$cert['n']]), 256), 'e' => new \Mailster\phpseclib\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$cert['e']]), 256)]);
    ...
    public function revoke($token, array $options = [])
    {
        ...
        $body = \Mailster\GuzzleHttp\Psr7\stream_for(\http_build_query(['token' => $token]));
        $request = new \Mailster\GuzzleHttp\Psr7\Request('POST', self::OAUTH2_REVOKE_URI, ['Cache-Control' => 'no-store', 'Content-Type' => 'application/x-www-form-urlencoded'], $body);
    ...
    private function retrieveCertsFromLocation($url, array $options = [])
    {
        ...
        $httpHandler = $this->httpHandler;
        $response = $httpHandler(new \Mailster\GuzzleHttp\Psr7\Request('GET', $url), $options);
    ...
    private function checkAndInitializePhpsec()
    {
        ...
        if (!\class_exists('Mailster\\phpseclib\\Crypt\\RSA')) {
    ...
    {
        // @codeCoverageIgnoreStart
        if (!\class_exists('Mailster\\SimpleJWT\\JWT')) {
    ...
    private function setPhpsecConstants()
    {
        if (\filter_var(\getenv('GAE_VM'), \FILTER_VALIDATE_BOOLEAN)) {
            ...
            if (!\defined('CRYPT_RSA_MODE')) {
                \define('CRYPT_RSA_MODE', \Mailster\phpseclib\Crypt\RSA::MODE_OPENSSL);
            }
        }
    }
    /**
    ...
    {
        $class = \class_exists('Mailster\\Firebase\\JWT\\JWT') ? 'Firebase\\JWT\\JWT' : 'JWT';
