 ...
 */
namespace Mailster\Google\Auth\Middleware;
...
class SimpleMiddleware
{
    ...
    {
        return function (\Mailster\Psr\Http\Message\RequestInterface $request, array $options) use($handler) {
            // Requests using "auth"="scoped" will be authorized.
            if (!isset($options['auth']) || $options['auth'] !== 'simple') {
                return $handler($request, $options);
            }
            $query = \Mailster\GuzzleHttp\Psr7\parse_query($request->getUri()->getQuery());
            $params = \array_merge($query, $this->config);
            $uri = $request->getUri()->withQuery(\Mailster\GuzzleHttp\Psr7\build_query($params));
